...
class AIPartsAgent {
    ...
    async getModelEnrichment(make, model) {
        ...
        try {
            ...
            // Complete loading bar even if no images
    ...
    async getManuals(make, model) {
        ...
        // Start loading bar for manual search
    ...
    async resolvePart(description, make, model) {
        try {
            ...
            // Start loading bar for part resolution (longest operation)
            ...
            // Use the intelligently recommended result if available (prioritize this!)
            if (data.recommended_result && data.recommended_result.oem_part_number) {
                ...
                // Complete loading bar on success
            ...
            if (results.manual_search && results.manual_search.found && results.manual_search.oem_part_number) {
                // Only use if verified, otherwise fall back to similar parts
                const validation = results.manual_search.serpapi_validation || {};
                if (validation.is_valid === true) {
                    console.log('✅ Using verified manual search result:', results.manual_search);
                    const result = { ...results.manual_search };
                    result.alternate_part_numbers = allAlternates;
                    return result;
                } else {
                    console.log('⚠️ Manual search result not verified, will check for similar parts fallback');
                }
            }
            
            if (results.ai_web_search && results.ai_web_search.found && results.ai_web_search.oem_part_number) {
                // Only use if verified, otherwise fall back to similar parts
                const validation = results.ai_web_search.serpapi_validation || {};
                if (validation.is_valid === true) {
                    console.log('✅ Using verified AI web search result:', results.ai_web_search);
                    const result = { ...results.ai_web_search };
                    result.alternate_part_numbers = allAlternates;
                    return result;
                } else {
                    console.log('⚠️ AI web search result not verified, will check for similar parts fallback');
                }
            }
            
            if (results.database && results.database.found && results.database.oem_part_number) {
                // Only use if verified, otherwise fall back to similar parts
                const validation = results.database.serpapi_validation || {};
                if (validation.is_valid === true) {
                    console.log('✅ Using verified database result:', results.database);
                    const result = { ...results.database };
                    result.alternate_part_numbers = allAlternates;
                    return result;
                } else {
                    console.log('⚠️ Database result not verified, will check for similar parts fallback');
                }
            }
            
            // Check if similar parts were triggered as a fallback (only when no recommended result)
            if (data.similar_parts_triggered && data.similar_parts && data.similar_parts.length > 0) {
                console.log('🔍 Similar parts search was triggered as fallback:', data.similar_parts);
                console.log('   Reason:', data.recommendation_reason);
                ...
                return {
                    ...
                    serpapi_validation: { is_valid: false }, // Similar parts fallback = not verified, so similar parts will display
            ...
            // Complete loading bar even if no results
    ...
    async getSuppliers(partNumber, make, model) {
        ...
        // Start loading bar for supplier search
        ...
        try {
            const response = await fetch(`${this.baseURL}/api/suppliers/search`, {
                ...
                body: JSON.stringify({
                    ...
                    model,
                    oem_only: false,
                    use_v2: true  // Use v2 with PartsTown boosting
                })
            });
    ...
    async initiatePurchase(supplier, partNumber) {
        try {
            ...
            let variables;
            try {
                variables = this.convertBillingProfileToVariables();
            } catch (error) {
                this.updatePurchaseStatus('No billing profile configured', 'fas fa-exclamation-triangle');
                this.showPurchaseActions([
                    { text: 'Create Profile', action: () => this.showSettings() },
                    { text: 'View Manually', action: () => window.open(supplier.url, '_blank') }
                ]);
                return;
            }
    ...
    updatePart(partData) {
        ...
        // Determine verification status based ONLY on validation (ignore similar_parts_triggered)
        ...
        if (isVerified) {
            ...
            `;
            // Show verified actions (alternate parts + generic parts buttons)
            verifiedActionsElement.style.display = 'block';
            // NEVER show similar parts for verified parts
            ...
            `;
            // Hide verified actions (no alternate/generic buttons for unverified)
            verifiedActionsElement.style.display = 'none';
            
            // FORCE display similar parts for unverified parts
                ...
                <div class="supplier-card-clean" data-supplier-index="${index}">
                    ${supplier.ai_ranking ? `
                        <div class="ai-rank-badge">
                            <i class="fas fa-brain"></i> #${index + 1}
                        </div>
                    ` : ''}
    ...
    convertBillingProfileToVariables() {
        ...
        if (!this.defaultBillingProfile) {
            this.addLog('error', 'Purchase', 'No billing profile found. Purchase automation requires a billing profile.');
            throw new Error('No billing profile configured. Please create a billing profile in settings before using purchase automation.');
        }
    ...
    getDefaultVariables() {
        // Return default variables for form placeholders and examples (NOT for purchase automation)
        // These values are DIFFERENT from dummy_values.json to prevent substitution conflicts
        return {
            first_name: "Jane",
            last_name: "Doe", 
    ...
    populateApiDocs() {
        const apiEndpoints = [
            // Manuals API
            {
                ...
                description: 'Search for technical manuals using AI-powered relevance scoring and automatic downloads. ⏱️ Average response: 3.8s',
                example: {
                    request: `{
  "make": "Carrier",
  "model": "58STA"
  ...
  "results": [
    {
      ...
      "verified": true
            ...
            },
            {
                method: 'GET',
                path: '/api/manuals',
                description: 'List all stored manuals with pagination support. ⏱️ Average response: 7.9ms',
                example: {
  ...
  "per_page": 10
            ...
            },
            {
                method: 'GET',
                path: '/api/manuals/{id}',
                description: 'Get a specific manual by ID. ⏱️ Average response: ~7ms',
                example: {
  ...
  "created_at": "2025-01-01T12:00:00Z"
            ...
            },
            {
                method: 'POST',
                path: '/api/manuals',
                description: 'Create a new manual entry. ⏱️ Average response: ~10ms',
                example: {
                    request: `{
  "make": "Carrier",
  "model": "58STA",
  ...
  "success": true
            ...
            // Parts API
            {
                ...
                description: 'Resolve generic part descriptions to OEM part numbers using AI and multiple search methods. ⏱️ Average response: 15.6s',
                example: {
                    request: `{
  ...
  "recommended_result": {...}
            ...
            },
            {
                method: 'GET',
                path: '/api/parts',
                description: 'List all stored parts with pagination. ⏱️ Average response: 7.0ms',
                example: {
  ...
  "per_page": 20
            ...
            },
            {
                ...
                description: '⚠️ ENDPOINT CURRENTLY UNAVAILABLE - Find similar or alternative parts when exact resolution fails. ⏱️ Status: Failed (400 errors)',
                example: {
                    request: `{
  ...
  "total_found": 1
            ...
            },
            {
                method: 'POST',
                path: '/api/parts',
                description: 'Create a new part entry. ⏱️ Average response: ~10ms',
                example: {
                    request: `{
  "oem_part_number": "HH18HA499",
  "description": "High limit switch",
  ...
  "message": "Part deleted successfully"
            ...
            // Generic Parts API
            {
                ...
                description: '⚠️ ENDPOINT CURRENTLY UNAVAILABLE - Find generic/aftermarket alternatives to OEM parts using GPT-4.1-Nano. ⏱️ Status: Failed (400 errors)',
                example: {
                    request: `{
  ...
  "search_metadata": {
    ...
    "ai_validated": 6
            ...
            },
            {
                ...
                description: 'Validate compatibility between an OEM part and a proposed generic alternative using AI analysis. ⏱️ Average response: 2.1ms',
                example: {
                    request: `{
  "oem_part_number": "HH18HA499",
  "generic_part_number": "C7027A1049",
  ...
  "risk_assessment": "Low risk - well-documented cross-reference"
            ...
            // Suppliers API
            {
                ...
                description: 'Find suppliers for specific OEM part numbers with AI-powered ranking and validation. ⏱️ Average response: 11.4s',
                example: {
                    request: `{
  "part_number": "HH18HA499",
  "make": "Carrier",
  ...
  "suppliers": [
    {
      ...
      "ai_ranking": 1
            ...
            },
            {
                method: 'GET',
                path: '/api/suppliers',
                description: 'List all stored suppliers. ⏱️ Average response: 5.3ms',
                example: {
  ...
  "message": "Supplier deleted successfully"
            ...
            // Purchases API
            {
                method: 'POST',
                path: '/api/purchases',
                description: 'Automate purchase execution using pre-recorded browser sessions with real billing profiles. ⏱️ Average response: 30-120s (varies by site)',
                example: {
                    request: `{
  "part_number": "HH18HA499",
  ...
  "confirmation_code": "CONF-456"
            ...
            },
            {
                method: 'GET',
                path: '/api/purchases',
                description: 'List all purchases with pagination. ⏱️ Average response: 2.7ms',
                example: {
  ...
  "purchase_id": 123
            ...
            // Profiles API
            {
                method: 'POST',
                path: '/api/profiles',
                description: 'Create encrypted billing profile for automated purchases. ⏱️ Average response: ~50ms',
                example: {
                    request: `{
  "name": "Main Office",
  "billing_address": {
  ...
  "message": "Profile created successfully"
            ...
            },
            {
                method: 'GET',
                path: '/api/profiles',
                description: 'List all billing profiles. ⏱️ Average response: 1.8ms',
                example: {
  ...
  "message": "Profile deleted successfully"
            ...
            // Enrichment API
            {
                method: 'POST',
                path: '/api/enrichment',
                description: 'Enrich part or equipment data with additional multimedia content using GPT-4.1-Nano analysis. ⏱️ Average response: 2.5s',
                example: {
                    request: `{
  "make": "Carrier",
  "model": "58STA",
    ...
    "images": [
      {
        ...
        "title": "High Limit Switch"
            ...
            // Screenshots API
            {
                ...
                description: 'Capture screenshots of supplier websites for preview. ⏱️ Average response: 887ms (varies by website)',
                example: {
                    request: `{
  "urls": [
  ...
  "screenshots": {
    "https://www.etundra.com/part": "screenshots/suppliers/etundra_abc123.png"
            ...
            // Recordings API
            {
                ...
                description: 'Get list of domains with available purchase automation recordings. ⏱️ Average response: 2.6ms',
                example: {
  ...
  "count": 3
            ...
            },
            {
                ...
                description: 'Health check endpoint for recording studio service. ⏱️ Average response: 1.8ms',
                example: {
  ...
  "message": "Recording session started. Interact with the browser and press Ctrl+C to stop."
            ...
            },
            {
                ...
                description: 'List all available recordings. ⏱️ Average response: 2.4ms',
                example: {
  ...
  "success": true
            ...
            },
            {
                ...
                description: 'Get current variables for purchase automation. ⏱️ Average response: 3.3ms',
                example: {
  ...
  "recording_name": "etundra"
            ...
            // System API
            {
                ...
                description: 'System health check endpoint for monitoring Flask API service status and database connectivity. ⏱️ Average response: ~2ms',
                example: {
  ...
  "response_time_ms": 2.5
            ...
            },
            {
                ...
                description: 'Clear cached files and temporary data. ⏱️ Average response: 19.3ms',
                example: {
  ...
  "cache_cleared": true
        ...
        // Add documentation links at the bottom
        container.innerHTML += `
    ...
    showSettings() {
        ...
        // Load billing profile data after a short delay to ensure DOM is ready
        setTimeout(() => {
        ...
        this.addLog('info', 'User Action', 'Opened settings screen');
    }
    ...
    async loadBillingProfile() {
        try {
            // Load the default billing profile
            console.log('Loading billing profile...');
            const response = await fetch('/api/profiles/1?include_sensitive=true');
            if (response.ok) {
                const profile = await response.json();
                console.log('Billing profile loaded:', profile);
                this.defaultBillingProfile = profile;
                this.populateBillingForm(profile);
                this.addLog('info', 'Settings', 'Billing profile loaded successfully');
            } else {
                console.log('No billing profile found, response status:', response.status);
                // If no profile exists, clear the form
                this.clearBillingForm();
                this.addLog('warning', 'Settings', 'No billing profile found - please create one');
            }
        } catch (error) {
            console.error('Error loading billing profile:', error);
            this.clearBillingForm();
            this.addLog('error', 'Settings', `Failed to load billing profile: ${error.message}`);
        }
    }
    ...
    clearBillingForm() {
        ...
        const formFields = [
            ...
            'billingAddress', 'billingAddress2', 'billingCity', 'billingState', 'billingStateAbr', 
            'billingZip', 'billingCountry', 'billingBillFirstName', 'billingBillLastName', 
            'billingBillAddress', 'billingBillCity', 'billingBillState', 'billingBillStateAbr', 
            'billingBillZip', 'billingCardNumber', 'billingCardExpMonth', 'billingCardExpYear', 
            'billingCardCvv', 'billingCardName'
        ];
    ...
    populateBillingForm(profile) {
        console.log('Populating billing form with profile:', profile);
        
        // Check if elements exist
        const firstNameElement = document.getElementById('billingFirstName');
        console.log('billingFirstName element exists:', !!firstNameElement);
        
        if (!firstNameElement) {
            console.error('Billing form elements not found - form may not be loaded yet');
            return;
        }
        ...
        // Personal Information
        document.getElementById('billingFirstName').value = billing.first_name || '';
        document.getElementById('billingLastName').value = billing.last_name || '';
        document.getElementById('billingEmail').value = billing.email || '';
        document.getElementById('billingPhone').value = billing.phone_number || '';
        document.getElementById('billingCompany').value = billing.company_name || '';
        
        console.log('Populated first name with:', billing.first_name);
        
        // Address Information
        document.getElementById('billingAddress').value = billing.address1 || '';
        ...
        document.getElementById('billingStateAbr').value = billing.state_abr || '';
        document.getElementById('billingZip').value = billing.zip || '';
        document.getElementById('billingCountry').value = billing.country || 'United States';
        
        // Billing Address (if different) - using payment_info for billing address
        document.getElementById('billingBillFirstName').value = billing.billing_first_name || '';
        document.getElementById('billingBillLastName').value = billing.billing_last_name || '';
        document.getElementById('billingBillAddress').value = payment.billing_address || '';
        document.getElementById('billingBillCity').value = payment.billing_city || '';
        document.getElementById('billingBillState').value = payment.billing_state || '';
        document.getElementById('billingBillStateAbr').value = billing.billing_state_abr || '';
        document.getElementById('billingBillZip').value = payment.billing_zip || '';
        
        // Credit Card Information
        document.getElementById('billingCardNumber').value = payment.card_number || '';
        document.getElementById('billingCardExpMonth').value = payment.card_exp_month || '';
        document.getElementById('billingCardExpYear').value = payment.card_exp_year || '';
        document.getElementById('billingCardCvv').value = payment.card_cvv || '';
        document.getElementById('billingCardName').value = payment.card_name || '';
    ...
    closeSettings() {
        ...
        this.showMainScreen();
        this.addLog('info', 'User Action', 'Closed settings screen');
    }
...
AIPartsAgent.prototype.updateGenericPartsButton = function(partData) {
    const genericSection = document.getElementById('genericPartsSection');
    // Show generic parts button for verified parts with OEM part number
    const validation = partData.serpapi_validation || {};
    const isVerified = validation.is_valid === true;
    
    if (genericSection && partData && partData.oem_part_number && isVerified) {
        genericSection.style.display = 'block';
        this.addLog('info', 'Generic Parts', 'Generic parts section shown - verified OEM part found');
    } else {
        genericSection.style.display = 'none';
        this.addLog('debug', 'Generic Parts', 'Generic parts section hidden - not verified or no OEM part');
    }
};
